...
The implemented prototype was extended beyond the initial grammar with several
user-facing features that are important for practical data exploration. The
filter language now supports compound boolean expressions with explicit
\texttt{AND} / \texttt{OR} precedence, string predicates
\texttt{starts\_with}, \texttt{contains}, and \texttt{ends\_with}, result-set
ordering through \texttt{sort}, row capping through \texttt{limit}, and an
interactive REPL for incremental execution. Duplicate detection also accepts
\texttt{levenshtein\_matcher} as an alias for the fuzzy duplicate matcher,
making the matching strategy clearer in scripts that are specifically based on
Levenshtein distance.

The prototype additionally introduces a \texttt{summarize} command that
produces a compact, aggregate \emph{proof} table over one or more pipeline
stages. Rather than appending more annotation columns to the data, it collapses
each named stage into a single row of metrics --- row count, column count,
duplicate-group and duplicate-candidate counts, typo-suspect count, and the
number of input rows represented after a merge --- so that the effect of a
cleaning pipeline (for example, ``30 raw rows reduced to 15 merged rows'') can
be exported and verified at a glance.
...
Commands
  & \texttt{scan},\quad \texttt{filter},\quad \texttt{merge},\quad
    ...
    \texttt{findLocalDuplicates},\quad \texttt{sort},\quad \texttt{limit},\quad
    \texttt{summarize} \\
\hline
Conditions \& Logic
  & \texttt{where},\quad \texttt{AND},\quad \texttt{OR},\quad
    \texttt{columns},\quad \texttt{starts\_with},\quad
    \texttt{contains},\quad \texttt{ends\_with} \\
\hline
Ordering
  & \texttt{by},\quad \texttt{asc},\quad \texttt{desc} \\
\hline
Tool Integration
...
\begin{lstlisting}[style=bnf, caption={Complete BNF grammar of DaQuVa.}, label={lst:bnf}]
  ...
  <and_condition>   ::= <expression> "AND" <and_condition>
                      | <expression>

  <expression>      ::= <identifier> <comparison_op> <value>
                      | <identifier> <string_match_op> <value>
  ...
  <string_match_op> ::= "starts_with"
                      | "contains"
                      | "ends_with"


  <sort_command>    ::= "sort" <identifier>
                        [ "by" ] <identifier> <opt_sort_direction>
  ...
  <limit_command>   ::= "limit" <identifier> <number>


  <summary_command> ::= "summarize" <identifier> <summary_tail>

  <summary_tail>    ::= "," <identifier> <summary_tail>
                      | <empty>
...
either compares a column-level attribute (e.g.\ \texttt{dirty\_level}) to a
value using a standard comparison operator, or applies one of the supported
string predicates. This deliberately limited expression language prevents
arbitrary logic and keeps programs auditable.
...
\subsubsection{\texttt{filter} --- Boolean and String Filtering}
...
with \texttt{AND} / \texttt{OR}; \texttt{AND} binds more tightly than
\texttt{OR}, matching standard boolean precedence. Predicates may compare
numeric or string values with \texttt{==}, \texttt{!=}, \texttt{>},
\texttt{<}, \texttt{>=}, and \texttt{<=}, or apply string tests with
\texttt{starts\_with}, \texttt{contains}, and \texttt{ends\_with}.
...
\subsubsection{\texttt{sort} and \texttt{limit} --- Result-Set Ordering}

\begin{lstlisting}[style=bnf]
<sort_command>  ::= "sort" <identifier> [ "by" ] <identifier> [ "asc" | "desc" ]
<limit_command> ::= "limit" <identifier> <number>
\end{lstlisting}

\textbf{Meaning.} \texttt{sort} produces a table ordered by a selected column,
while \texttt{limit} returns only the first $n$ rows. Together they support
common exploration workflows such as inspecting the oldest customers, highest
scores, or first few suspicious records.
...
\textbf{Execution.} In-memory tables are ordered by a stable mixed-type sort
key so that empty, numeric, boolean, and string values can be handled without
runtime type errors. When the source plan is backed by SQLite, sorting and
limiting are pushed down into \texttt{ORDER BY} and \texttt{LIMIT} clauses.

\subsubsection{\texttt{summarize} --- Aggregate Proof Summary}

\begin{lstlisting}[style=bnf]
<summary_command> ::= "summarize" <identifier> { "," <identifier> }
\end{lstlisting}

\textbf{Meaning.} \texttt{summarize} takes one or more previously bound result
sets (the \emph{stages} of a pipeline) and produces a single new table in which
\emph{each named stage becomes one row of aggregate metrics}. Unlike every other
command, it does not add columns to the data --- it reduces each stage to a
fixed schema of counts, giving an inspectable proof that a transformation
actually happened.

\textbf{Result schema.} The produced table has a fixed set of columns:
\texttt{stage}, \texttt{row\_count}, \texttt{column\_count},
\texttt{duplicate\_group\_count}, \texttt{duplicate\_candidate\_count},
\texttt{typo\_suspect\_count}, \texttt{input\_rows\_represented}, and
\texttt{rows\_removed\_by\_merge}. The last two are derived from the
\texttt{merged\_from\_count} annotation so that a merge which collapses several
input rows into one is reported as rows represented versus rows removed.
...
\textbf{Execution.} For each stage, the engine materialises the underlying plan
and computes the metrics in a single pass over the rows; duplicate groups are
counted from distinct \texttt{duplicate\_group\_id} values, typo suspects from a
truthy \texttt{typo\_suspect} flag, and represented input rows from the merge
count. The resulting summary is an ordinary result set, so it can be exported to
a CSV file or persisted to SQLite like any other table.

\textbf{Errors.} An error is raised if any referenced stage variable is
undefined, using the same \texttt{UndefinedVariable} condition as the other
commands.
...
AI-powered tool (e.g.\ a fuzzy matcher) provides similarity scoring. The
implementation accepts both \texttt{fuzzy\_duplicates} and
\texttt{levenshtein\_matcher}; the latter is an explicit alias for the same
Levenshtein-distance based duplicate detection strategy.
...
list. Function bodies may return either a value/variable or the direct result
of another function call, which allows small helper functions to delegate to
...
# === 5. Explore high-risk records with string filters and top-N ordering ===
major_providers = filter high_risk_emails
                  where email contains "@gmail" OR email contains "@outlook";
top_risk = sort high_risk_emails by dirty_level desc;
top_risk = limit top_risk 5;
output major_providers -> console;
output top_risk -> console;

# === 6. Detect near-duplicate records among high-risk rows ===
duplicates = findLocalDuplicates high_risk_emails
             columns ["name", "dob"]
             using tool "levenshtein_matcher";

# === 7. Merge duplicates back into the database (confidence >= 90%) ===
merge duplicates -> Here(90) allowDanger;

# === 8. Create a summary table for downstream analysis ===
analysis_table = newTable "high_risk_analysis"
                 from high_risk_emails
                 addColumns [duplicate_score, length("name")];

# === 9. Additional validation pass on the summary table ===
...
\begin{itemize}
    ...
    \item \texttt{contains}, \texttt{sort}, and \texttt{limit} support
          interactive exploration without leaving the DSL.
...
duplicate detection and merging, result ordering, aggregate proof summaries,
result export, and table construction.
...
and the expression language is limited to comparisons, boolean composition, and
auditable string predicates rather than arbitrary side-effecting code. These
properties make DaQuVa programs straightforward to review, test, and extend.
